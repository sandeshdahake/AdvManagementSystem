/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, async, inject, fakeAsync, tick } from '@angular/core/testing';
import { NgbActiveModal } from '@ng-bootstrap/ng-bootstrap';
import { Observable } from 'rxjs/Rx';
import { JhiEventManager } from 'ng-jhipster';

import { AdvManagementSystemTestModule } from '../../../test.module';
import { ClientSubscriptionDialogComponent } from '../../../../../../main/webapp/app/entities/client-subscription/client-subscription-dialog.component';
import { ClientSubscriptionService } from '../../../../../../main/webapp/app/entities/client-subscription/client-subscription.service';
import { ClientSubscription } from '../../../../../../main/webapp/app/entities/client-subscription/client-subscription.model';
import { CityService } from '../../../../../../main/webapp/app/entities/city';
import { ClientService } from '../../../../../../main/webapp/app/entities/client';
import { SubscriptionPlanService } from '../../../../../../main/webapp/app/entities/subscription-plan';

describe('Component Tests', () => {

    describe('ClientSubscription Management Dialog Component', () => {
        let comp: ClientSubscriptionDialogComponent;
        let fixture: ComponentFixture<ClientSubscriptionDialogComponent>;
        let service: ClientSubscriptionService;
        let mockEventManager: any;
        let mockActiveModal: any;

        beforeEach(async(() => {
            TestBed.configureTestingModule({
                imports: [AdvManagementSystemTestModule],
                declarations: [ClientSubscriptionDialogComponent],
                providers: [
                    CityService,
                    ClientService,
                    SubscriptionPlanService,
                    ClientSubscriptionService
                ]
            })
            .overrideTemplate(ClientSubscriptionDialogComponent, '')
            .compileComponents();
        }));

        beforeEach(() => {
            fixture = TestBed.createComponent(ClientSubscriptionDialogComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(ClientSubscriptionService);
            mockEventManager = fixture.debugElement.injector.get(JhiEventManager);
            mockActiveModal = fixture.debugElement.injector.get(NgbActiveModal);
        });

        describe('save', () => {
            it('Should call update service on save for existing entity',
                inject([],
                    fakeAsync(() => {
                        // GIVEN
                        const entity = new ClientSubscription(123);
                        spyOn(service, 'update').and.returnValue(Observable.of(entity));
                        comp.clientSubscription = entity;
                        // WHEN
                        comp.save();
                        tick(); // simulate async

                        // THEN
                        expect(service.update).toHaveBeenCalledWith(entity);
                        expect(comp.isSaving).toEqual(false);
                        expect(mockEventManager.broadcastSpy).toHaveBeenCalledWith({ name: 'clientSubscriptionListModification', content: 'OK'});
                        expect(mockActiveModal.dismissSpy).toHaveBeenCalled();
                    })
                )
            );

            it('Should call create service on save for new entity',
                inject([],
                    fakeAsync(() => {
                        // GIVEN
                        const entity = new ClientSubscription();
                        spyOn(service, 'create').and.returnValue(Observable.of(entity));
                        comp.clientSubscription = entity;
                        // WHEN
                        comp.save();
                        tick(); // simulate async

                        // THEN
                        expect(service.create).toHaveBeenCalledWith(entity);
                        expect(comp.isSaving).toEqual(false);
                        expect(mockEventManager.broadcastSpy).toHaveBeenCalledWith({ name: 'clientSubscriptionListModification', content: 'OK'});
                        expect(mockActiveModal.dismissSpy).toHaveBeenCalled();
                    })
                )
            );
        });
    });

});
